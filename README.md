...
- Part one, which deals with images, which is further divided into:
  ...
  - Model Comparison, we use two models here:
    - MLP: generated suboptimal results, as the flattening operation limits the model's ability to learn the shape and form of an object 
...
- Part two, which deals with sentiment analysis for the text, which is further divided into:
  ...
  - Preprocessing and tokenization pipeline: 
    1. cleaned and normalized the data
    2. tokenization of the data: used spacy in order to tokenize the data, each word has an unique index, 0 is reserved for padding and 1 for out-of-vocabulary (OOV) tokens (not in the train dataset)
    3. the usage of an pretrained embedding layer (FastText), which represents words as 300-element vectors
    4. sequence padding, we made them all to have the global maximum length
    5. the embedding matrix, here we transformed the words from our sets into embeddings, and represented unknown words, with a 300-element vector which had random values ranging from -1 to 1
